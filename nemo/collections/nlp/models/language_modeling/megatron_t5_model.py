...
class MegatronT5Model(NLPModel):
    ...
    def get_forward_output_and_loss_func(self):
        if self.megatron_amp_o2:
            ...
            def fwd_output_and_loss_func(batch, model):
                encoder_input_ids, decoder_input_ids, loss_mask, lm_labels, encoder_attn_mask, decoder_attn_mask, encoder_decoder_attn_mask = batch
                result, _ = model(
                    encoder_input_ids=encoder_input_ids,
                    decoder_input_ids=decoder_input_ids,
                    encoder_attn_mask=encoder_attn_mask,
    ...
    def process_global_batch(self, global_batch):
        ...
        for micro_batch in global_batch:
            text_enc, text_dec, labels, loss_mask, enc_mask, dec_mask, enc_dec_mask = self.process_micro_batch(micro_batch)
            ...
            enc_dec_mask_repeat = torch.concat([enc_dec_mask for _ in range(micro_batch_size)])
            enc_mask_list.append(enc_mask_repeat)
            dec_mask_list.append(dec_mask_repeat)
            enc_dec_mask_list.append(enc_dec_mask_repeat)
        ...
        enc_dec_mask_tensor = torch.concat(enc_dec_mask_list, dim=0)

        return tokens_enc_tensor, tokens_dec_tensor, labels_tensor, loss_mask_tensor, enc_mask_tensor, dec_mask_tensor, enc_dec_mask_tensor
    ...
    def build_pretraining_data_loader(self, dataset, consumed_samples):
        ...
        if hasattr(self.cfg.data, 'dataloader_type') and self.cfg.data.dataloader_type is not None:
            if self.cfg.data.dataloader_type == 'single':
                ...
                raise ValueError(f'cfg.data.dataloader_type must be "single" or "cyclic" got {self.cfg.data.dataloader_type}')
        else:
            raise ValueError(f'cfg.data.dataloader_type must be "single" or "cyclic" got {self.cfg.data.dataloader_type}')
